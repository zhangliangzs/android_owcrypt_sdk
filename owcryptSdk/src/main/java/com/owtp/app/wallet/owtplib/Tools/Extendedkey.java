...
public class Extendedkey {
    ...
    public Extendedkey GenPrivateChild(Extendedkey k,int serializes)
    {
        ...
        if( new BigInteger(String.valueOf(serializes)).abs().compareTo(HardenedKeyStart)>=0) { //强化扩展
    ...
    {
        if (!k.isPrivate) {
            if (new BigInteger(String.valueOf(serializes)).abs().compareTo(HardenedKeyStart)>=0 ){ //不能从父公钥强化扩展
                return null;
            }
    ...
    public byte[] getPriChildViaPriParent(byte[] il,byte[] prikey ,int typeChoose) {
        ...
        if (typeChoose == EccSet.ECC_CURVE_ED25519) {
            ...
            byte[] index=getIndexbyte(0,28,il);
            BigInteger ilNum = new BigInteger(byteArrayToHex(inverse(index)),16);
            BigInteger kpr =  new BigInteger(byteArrayToHex(inverse(prikey)),16);
            BigInteger num8 = new BigInteger(new byte[]{8});
            BigInteger  curveOrder = new BigInteger(byteArrayToHex(EccSet.EccGetCurveOrder(typeChoose)),16);
            ilNum=ilNum.multiply(num8);
            ilNum=ilNum.add(kpr);
            BigInteger check=ilNum.mod(curveOrder);
            if (check.signum() == 0) {
                return null;
            }
            priChild = inverse(ilNum.toByteArray());
        } else {
            ...
            priChild =getIndexbyte(ilNum.toByteArray().length-32,ilNum.toByteArray().length,ilNum.toByteArray());
        }
        return priChild;
    }
